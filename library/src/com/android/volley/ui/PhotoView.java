        ...
        HorizontallyScrollable
{
     ...
     */
    public void bindPhoto(BitmapDrawable photoBitmap)
    {
        boolean changed = false;
        if (mDrawable != null)
        {
            if (photoBitmap == mDrawable)
            {
            ...
            changed = photoBitmap != null &&
                    (mDrawable.getIntrinsicWidth() != photoBitmap.getIntrinsicWidth() ||
                            mDrawable.getIntrinsicHeight() != photoBitmap.getIntrinsicHeight());
    ...
    public void loadImageIfNecessary(final boolean isInLayoutPass)
    {
        ...
        ImageContainer newContainer = mImageLoader.get(mUrl,
                new ImageListener()
                {
                    ...
                    @Override
                    public void onResponse(final ImageContainer response, boolean isImmediate, final boolean isFinished)
                    {
                        ...
                        if (isImmediate && isInLayoutPass)
                        {
                            post(new Runnable()
                            {
                                ...
                                public void run()
                                {
                                    onResponse(response, false, isFinished);
                                }
                            });
                            return;
                        }
